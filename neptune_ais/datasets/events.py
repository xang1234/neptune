...
``"low"`` and ``"medium"`` are half-open ``[lo, hi)`` intervals.
``"high"`` is closed ``[lo, hi]`` (includes 1.0).

This matches ``classify_confidence()`` semantics. For Polars filtering,
use ``>=`` on the lower threshold directly::
